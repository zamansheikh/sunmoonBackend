...
import { IBundle, IStoreItem, IStoreItemDocument, IStoreItemModel } from "../../models/store/store_item_model";
...
export interface IStoreItemRepository {
    ...
    getAllStoreItems(category: string, query: Record<string, any>): Promise<{pagination: IPagination, items: IStoreItemDocument[]}>;
    updateStoreItem(id: string, item: Partial<IStoreItem>): Promise<IStoreItemDocument>;
    deleteStoreItem(id: string, days: number): Promise<IStoreItemDocument>;
    updateBundleCategoryTitle(oldTitle: string, newTitle: string): Promise<UpdateResult>;
...
export default class StoreItemRepository implements IStoreItemRepository {
    ...
    async getAllStoreItems(category:string, query: Record<string, any>): Promise<{ pagination: IPagination; items: IStoreItemDocument[]; }> {
        const qb = new QueryBuilder(this.Model, query);
        const res = qb.find({categoryId: category, deleteStatus: false}).sort().paginate();
    ...
    async deleteStoreItem(id: string, days: number): Promise<IStoreItemDocument> {
        ...
        const deleted = await this.Model.findByIdAndUpdate(id, { expireAt: newDate, deleteStatus: true }, { new: true });
        if (!deleted) throw new AppError(StatusCodes.NOT_FOUND, `Store item with id ${id} not found`);
        return deleted;
    }

    async updateStoreItem(id: string, item: Partial<IStoreItem>): Promise<IStoreItemDocument> {
        const updated = await this.Model.findByIdAndUpdate(id, item, { new: true });
        if (!updated) throw new AppError(StatusCodes.NOT_FOUND, `Store item with id ${id} not found`);
        return updated;
    }

    async updateBundleCategoryTitle(oldTitle: string, newTitle: string): Promise<UpdateResult> {
    ...
    async createNewBundles(id: string, bundle: IBundle[]): Promise<IStoreItemDocument> {
        const updated = await this.Model.findByIdAndUpdate(id, { $push: { bundleFiles: { $each: bundle } } }, { new: true });
        if (!updated) throw new AppError(StatusCodes.NOT_FOUND, `Store item with id ${id} not found`);
        return updated;
    }

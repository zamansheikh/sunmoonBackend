...
import {
  IPostsReaction,
  IPostsReactionModel,
} from "../../../entities/posts/posts_reaction_interface";
import { IPostReactionRepository } from "./post_reaction_repository_interface";

export default class PostsReactionRepostitory
  implements IPostReactionRepository
{
  ...
  async deleteUserReaction(userId: string): Promise<DeleteResult> {
    return await this.PostReactionModel.deleteMany({ reactedBy: userId });
  }
  ...
  async findPostReactionsConditionally(
    condition: Record<string, string | number>
  ) {
    return await this.PostReactionModel.find(condition);
  }

  async findPostReactopnByIdAndUpdate(
    id: string,
    payload: Record<string, any>
  ) {
    return await this.PostReactionModel.findByIdAndUpdate(id, payload, {
      new: true,
    });
  }
